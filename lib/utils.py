...
def cal_angle(a,b):
    ...
    if len(b.shape) == 1:
        b=b[:,np.newaxis]
    c = np.dot(a.T,b)
    return float(c/((np.linalg.norm(a)*np.linalg.norm(b)) + 1e-6))
...
def putText(img, label, score, left, top, textColor=(0, 255, 0), textSize=20):
    if (isinstance(img, np.ndarray)):  # 判断是否OpenCV图片类型
        img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    # 创建一个可以在给定图像上绘图的对象
    draw = ImageDraw.Draw(img)
    # 字体的格式
    #font =FontProperties(fname=, size=10)
    fontStyle = ImageFont.truetype(
        "/usr/share/fonts/truetype/arphic/uming.ttc", textSize, encoding="utf-8")
    # 绘制文本
    draw.text((left, top), '{}: {:.0%}'.format('1', score), textColor, font=fontStyle)
    # 转换回OpenCV格式
...
def plot_one_box(img, coord, label=None, score=None, color=None, line_thickness=None):
    ...
    #     c2 = c1[0] + t_size[0] + s_size[0] + 15, c1[1] - t_size[1] +25
    #     cv2.rectangle(img, c1, c2, color, -1)  # filled
    #     #img = putText(img,label,score,c1[0],c1[1]-2)
    #     cv2.putText(img, '{}: {:.0%}'.format(label, score), (c1[0], c1[1]+10), 0, float(tl) / 3, [0, 0, 0],
